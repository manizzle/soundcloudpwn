...
{    
        ...
        scdlr.JQ.getJSON("https://api.soundcloud.com/resolve.json?callback=?", 
                         {url: resolveUrl, client_id:scdlr.clientId, secret_token:secretToken},
                         function(track)
                            {                
                                trackId = track.id.toString();
                                scdlr.JQ.getJSON("https://api.soundcloud.com/i1/tracks/"+trackId+"/streams?callback=?",
                                                 {client_id:scdlr.clientId, secret_token:secretToken}, 
                                                 function( data )
                                                     {
...
scdlr.JQ(".sound").not(".playlist").each( 
    function()
        {
...
{
    scdlr.JQ(".trackList .trackList__listItem").each( 
        function ()
            {
                scdlr.addDownloadButton( this );
            }
